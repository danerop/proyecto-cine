...
<body>
					...
					<div class="list-group" id="list-tab" role="tablist">
						<c:forEach items="${listaSalas}" var="sala">
							<a class="list-group-item list-group-item-action"
								data-toggle="list" href="#list-sala${sala.getId()}" role="tab">Sala
								id ${sala.getId()} del cine: ${sala.getCine().getNombreLocal()}</a>
						</c:forEach>
					</div>
				</div>
				<div class="col-8">
					...
					<div class="tab-content">
						<c:forEach items="${listaSalas}" var="sala">
							<div class="tab-pane fade p-3 mb-2 bg-white rounded"
								id="list-sala${sala.getId()}" role="tabpanel">
								<div class="row">
									<div class="col-8">
										<span style="font-size: small; font-weight: lighter;">Sala
											id:${sala.getId()}</span> <span
											style="font-size: large; font-weight: bold;">Pertenece
											a ${sala.getCine().getNombreLocal()}</span>
	...
	<div class="container formularios bg-light rounded"
		...
		<div class="formulario-cargar-sala" style="padding: 1rem;">
			...
			<form:form action="agregar-sala" method="POST"
				modelAttribute="datosSala">
								...
								<tr>
									...
									<c:forEach var="col" begin="1" end="32">
										<td><input type="checkbox"
											class="btn-check position-fixed" name="butacas"
											id="btncheck${col+(32*(fil-1))}" autocomplete="off"
											value="${col+(32*(fil-1))}"> <form:label
												class="btn btn-outline-primary p-2"
												for="btncheck${col+(32*(fil-1))}" path="butacas">
                        &nbsp;&nbsp;&nbsp;</form:label></td>
									</c:forEach>
								</tr>
							</c:forEach>
